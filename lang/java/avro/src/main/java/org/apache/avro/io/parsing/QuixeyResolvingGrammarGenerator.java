 ...
 * schemas under Quixey's rules
 *
 * 1. no existing fields are modified
 * 2. added fields have default values
 */
public class QuixeyResolvingGrammarGenerator extends ValidatingGrammarGenerator {
  /**
  ...
  private Symbol resolveUnion(Schema writer, Schema reader,
     ...
     * {@link org.apache.avro.io.parsing.Symbol#production} for the reason.
     */
    int i = 0;
    for (Schema w : alts) {
  ...
  private Symbol resolveRecords(Schema writer, Schema reader,
    ...
    if (result == null) {
      ...
      if (rfields.size() < wfields.size()) {
        result = Symbol.error("fields were removed");
        seen.put(wsc, result);
      }

      // verify order has not changed and only new fields added
      int i = 0;
      for (i = 0; i < wfields.size(); i++) {
        if (!wfields.get(i).equals(rfields.get(i))) {
          result = Symbol.error("fields were modified");
          seen.put(wsc, result);
        }
      }
   ...
   * <code>seen</code> by {@link org.apache.avro.io.parsing.ValidatingGrammarGenerator#validating validating()}
   * from those put in by {@link org.apache.avro.io.parsing.ValidatingGrammarGenerator#resolving resolving()}.
   */
